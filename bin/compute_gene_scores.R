...
calculateSingscores <- function(xp, genes, raw=FALSE, ...) {
  ...
  # sscore <- lapply(geneSets, function(x) simpleScore(ranked.exp,
  #                                                 upSet=x$upSet,
  #                                                 downSet=x$downSet,
  ...
  sscore.df <- lapply(names(sscore),
                      function(x) flattenSingScores(sscore[[x]],x))
...
calculatePLAGEscores <- function(xp, genes, correct.dir=FALSE, ...) {
  ...
  # TODO: possibly implement a sign correction based on median, mean, or similar to get more consistent scores
  if(correct.dir) {
...
Samsscore <- function(xp, x, center_rows = TRUE,
                      geom_mean   = FALSE,
                      ...) {
...
# legacy function
correlationMatrixOld <- function(plageScores = NULL,
                                 gsvaScores = NULL,
                                 ssgseaScores = NULL,
                                 paiScores = NULL,
                                 singScores = NULL) {
...
boxplotScores <- function(score.matrix, set, group, title="") {
  bp.df <- data.frame(score=score.matrix[set,],
                      group=group)
