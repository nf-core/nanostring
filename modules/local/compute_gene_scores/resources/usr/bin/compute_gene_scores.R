...
flattenGenesets <- function(set) {
    ...
    if (is.null(set$downSet)) {
        return(set[['upSet']])
    } else {
        return(unique(c(set[['upSet']], set[['downSet']])))
    }
}
...
geneSetAsList <- function(set) {
    retSet <- list(upSet = character(), downSet = character(), knownDir = FALSE)
    if (is.character(set)) {
        retSet$upSet <- set
        retSet$downSet <- NULL
        retSet$knownDir <- FALSE
    } else if (is.data.frame(set)) {
        if (!is.numeric(set[, 2])) {
    ...
    if (length(retSet$upSet) == 0) {
        retSet$upSet <- NULL
    }
    if (length(retSet$downSet) == 0) {
        retSet$downSet <- NULL
    }
    return(retSet)
}
...
Singscore <- function(d, x, ...) {
    if (is.list(x) && is.null(x$downSet)) {
        s <- simpleScore(d, upSet = x$upSet, knownDirection = x$knownDir, ...)
    } else if (is.list(x) && !is.null(x$downSet)) {
        s <- simpleScore(
            d,
            upSet = x$upSet,
            downSet = x$downSet,
            knownDirection = x$knownDir,
            ...
        )
    }
    return(s)
...
calculateSingscores <- function(xp, genes, raw = FALSE, ...) {
    ...
    sscore.df <- lapply(names(sscore), function(x) {
        flattenSingScores(sscore[[x]], x)
    })
...
calculateGSVAscores <- function(xp, genes, ...) {
    ...
    if (is.data.frame(xp)) {
        xp.matrix <- as.matrix.data.frame(xp)
    }
    if (is.matrix(xp)) {
        xp.matrix <- xp
    }

    scores <- gsva(gsvaParam(xp.matrix, genes.flat), ...)
    return(scores)
}
...
calculateSSGSEAscores <- function(xp, genes, ...) {
    ...
    scores <- gsva(ssgseaParam(xp.matrix, genes.flat), ...)
    return(scores)
}
...
calculatePLAGEscores <- function(xp, genes, correct.dir = FALSE, ...) {
    ...
    plagescores <- gsva(plageParam(xp.matrix, genes.flat), ...)

    if (correct.dir) {
        meanscores <- calculateMeanScores(xp.matrix, genes.flat)
        i <- intersect(rownames(meanscores), rownames(plagescores))
        if (length(i) != length(rownames(plagescores))) {
            stop("Correction failed")
        }
        plagescores <- plagescores *
            sign(sapply(i, function(x) cor(plagescores[x, ], meanscores[x, ])))
    }
    return(plagescores)
}
...
calculateMedianScores <- function(xp, genes, ...) {
    ...
    median.result <- t(sapply(genes.flat, function(x) {
        apply(xp.matrix[x, ], 2, median, ...)
    }))
...
calculateMeanScores <- function(xp, genes, ...) {
    ...
    mean.result <- t(sapply(genes.flat, function(x) {
        apply(xp.matrix[x, ], 2, mean, ...)
    }))
    return(mean.result)
}
...
calculateSamsscores <- function(
    ...
    samsscore <- lapply(geneSets, function(x) {
        Samsscore(xp, x, center_rows = center_rows, geom_mean = geom_mean, ...)
    })
    if (raw) {
        return(samsscore)
    }
    ...
    score_flat_join <- Reduce(
        function(...) dplyr::full_join(..., by = "sample"),
        score_flat
    )

    ret_val <- score_flat_join %>%
    ...
    return(ret_val[, colnames(xp), drop = FALSE])
}
...
sams_score <- function(
    ...
    # method "sams" or "median", median is just the column median

    expr_mat <- as.matrix(expr_mat)
    assertthat::assert_that(
        is.numeric(expr_mat),
        msg = "expression matix is not numeric"
    )

    assertthat::assert_that(
        all(up_genes %in% rownames(expr_mat)),
        msg = "At least one element in up_genes is not in rownames of expression matrix."
    )
    assertthat::assert_that(
        all(down_genes %in% rownames(expr_mat)),
    ...
    down_cens[down_matrix > 0] <- matrix(
        0,
        nrow = nrow(down_matrix),
        ncol = ncol(down_matrix)
    )[down_matrix > 0]
...
calculateSignatureScores <- function(
    xp,
    genes,
    method,
    phenotypes = NULL,
    ...
    if (length(gene.sets.out) > 0) {
        warning(
            "The following genesets show no overlap with the expression matrix: ",
            paste0(gene.sets.out, collapse = ", "),
            call. = F
        )
    }
    ...
    if (length(genes) == 0) {
        stop(
            "No genesets show an overlap with the expression matrix. Please check if identifiers match."
        )
    }
...
multipleSignatureScores <- function(xp, genes, methods) {
    ...
    scores <- lapply(methods, function(x) {
        calculateSignatureScores(xp, genes, x)
    })
...
correlationMatrixOld <- function(
    ...
    if (!is.null(plageScores)) {
        plageScores <- reformatRownames(plageScores, "PLAGE")
    }
    if (!is.null(gsvaScores)) {
        gsvaScores <- reformatRownames(gsvaScores, "GSVA")
    }
    if (!is.null(ssgseaScores)) {
        ssgseaScores <- reformatRownames(ssgseaScores, "SSGSEA")
    }
    if (!is.null(paiScores)) {
        paiScores <- reformatRownames(paiScores, "PAI")
    }
    if (!is.null(singScores)) {
        singScores <- reformatRownames(singScores, "Singscore")
    }

    combined.matrix <- rbind(
        plageScores,
        gsvaScores,
        ssgseaScores,
        paiScores,
...
if (length(args) < 3) {
    stop(
        "Usage: process_nanostring_data.R <scores.yaml> <counts.tsv> algorithm",
        call. = FALSE
    )
}
...
scores <- calculateSignatureScores(
    xp = counts,
    genes = yaml_parsed,
    method = score.method
)
...
# description: 'Signature Scores: Algorithm: ",
    score.method,
    "'
# plot_type: 'heatmap'
# section_href: 'https://github.com/nf-core/nanoflow'"
